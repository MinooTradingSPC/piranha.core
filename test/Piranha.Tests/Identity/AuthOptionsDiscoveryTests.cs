...
public class AuthOptionsDiscoveryTests : IdentityAuthTestBase
{
    ...
    [Fact]
    public async Task AccountWithConfirmedPasskey_IsFoundByUserName()
    {
        // The login page's field is labelled "Username", so discovery must
        // resolve a username as well as an email.
        User user;
        using (var scope = Provider.CreateScope())
        {
            var userManager = scope.ServiceProvider.GetRequiredService<UserManager<User>>();
            user = new User { UserName = "passkey-by-name", Email = "passkey-by-name@example.com", EmailConfirmed = true };
            Assert.True((await userManager.CreateAsync(user, "Correct-Horse-1!")).Succeeded);
        }
        ...
        using (var scope = Provider.CreateScope())
        {
            ...
            var completeResult = await controller.CompleteRegistration(new CompletePasskeyRegistrationRequest
            {
                Token = challenge.Token,
                DeviceName = "Test device",
                AttestationResponse = new AuthenticatorAttestationRawResponse { Id = "id", RawId = new byte[] { 8, 8, 8 } }
            });
            Assert.IsType<Microsoft.AspNetCore.Mvc.OkObjectResult>(completeResult);
        }

        var response = await GetOptionsAsync("passkey-by-name");
